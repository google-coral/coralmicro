...
void IperfStart(struct jsonrpc_request* request) {
    ...
    if (server) {
        if (udp) {
            iperf_ctx.session =
                lwiperf_start_udp_server(&ip_addr_any, LWIPERF_TCP_PORT_DEFAULT,
        ...
        if (ipaddr_aton(server_ip_address.c_str(), &addr) == -1) {
            jsonrpc_return_error(
                request, -1, "failed to parse `server_ip_address`", nullptr);
            return;
        }
        if (udp) {
...
void IperfInit() {
    ...
    iperf_ctx.poll_timer =
        xTimerCreate("UDP poll timer", 1 / portTICK_PERIOD_MS, pdTRUE, nullptr,
                     timer_poll_udp_client);
