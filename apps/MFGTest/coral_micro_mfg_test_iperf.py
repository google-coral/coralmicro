...
if __name__ == '__main__':
    ...
    if args.s:
        ...
        response = mfg_test.iperf_start(is_server=True, udp=args.u, bandwidth=bandwidth)
        print(response)
    else:  # args.c
        print(f'Device ({args.device_ip_address}) acting as iPerf client, connecting to {args.c}')
        response = mfg_test.iperf_start(is_server=False, server_ip_address=args.c, duration=args.t, udp=args.u, bandwidth=bandwidth)
        print(response)
