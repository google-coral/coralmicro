...
void Main() {
  ...
  CHECK(LfsDirExists("/dir1"));
  CHECK(LfsDirExists("/dir2"));
  CHECK(LfsDirExists("/dir1/dir11"));
  CHECK(LfsDirExists("/dir1/dir13"));
  CHECK(LfsDirExists("/dir2/dir21"));

  CHECK(!LfsDirExists("/nonexistent"));
  ...
  std::string readstr;
  CHECK(LfsReadFile("/dir2/file2", &readstr));
  CHECK(readstr.length() == std::strlen(kFile2Str));

  constexpr lfs_soff_t kSeekOffset = 6;
  CHECK(lfs_file_open(Lfs(), &file1, "/dir1/file1", LFS_O_RDONLY) >= 0);
  CHECK(lfs_file_seek(Lfs(), &file1, kSeekOffset, LFS_SEEK_SET) == kSeekOffset);
  ...
  CHECK(
      lfs_file_write(Lfs(), &file2, kFile2Append, std::strlen(kFile2Append)) ==
      static_cast<lfs_ssize_t>(std::strlen(kFile2Append)));
  CHECK(lfs_file_size(Lfs(), &file2) ==
        static_cast<lfs_ssize_t>(std::strlen(kFile2Str) +
                                 std::strlen(kFile2Append)));
