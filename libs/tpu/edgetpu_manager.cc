...
EdgeTpuPackage* EdgeTpuManager::RegisterPackage(const char* package_content,
    ...
    flatbuffers::Verifier package_verifier(
        (const uint8_t*)package_binary.c_str(), package_binary.length());
    if (!package_verifier.VerifyBuffer<platforms::darwinn::Package>()) {
        printf("Package verification failed.\r\n");
        return nullptr;
    }

    auto* package = flatbuffers::GetRoot<platforms::darwinn::Package>(
        package_binary.c_str());
    if (flatbuffers::VectorLength(package->serialized_multi_executable()) ==
        0) {
    ...
    for (const auto* executable_serialized :
        ...
        if (executable->type() ==
            ...
            executable->type() ==
                platforms::darwinn::ExecutableType_STAND_ALONE) {
...
TfLiteStatus EdgeTpuManager::Invoke(EdgeTpuPackage* package,
    ...
    if (package->parameter_caching_exe()) {
        ...
        if (token != current_parameter_caching_token_) {
            ...
            package->parameter_caching_exe()->Invoke(tpu_driver_, context,
                                                     node);
            ...
            for (auto& cached_package : cached_packages_) {
                if (cached_package == package) {
                    ...
                    package->parameter_caching_exe()->Invoke(tpu_driver_,
                                                             context, node);
