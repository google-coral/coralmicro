    ...
    for (const auto *hint : *(executable_->dma_hints()->hints())) {
        switch (hint->any_hint_type()) {
            ...
            case platforms::darwinn::AnyHint_InstructionHint:
                ins_idx = hint->any_hint_as_InstructionHint()->instruction_chunk_index();
                bitstream = executable_->instruction_bitstreams()->Get(ins_idx)->bitstream();
                RETURN_IF_ERROR(tpu_driver.SendInstructions(bitstream->data(), bitstream->size()));
                break;
            default:
                break;
        }
...
OutputLayer::YBufferIndex OutputLayer::GetYBufferIndex(int y) const {
    const auto& layout = output_layer_->any_layer_as_OutputLayer()->layout();
    YBufferIndex output;
    output.y_linearized_tile_id = layout->y_coordinate_to_linear_tile_id_map()->Get(y);
