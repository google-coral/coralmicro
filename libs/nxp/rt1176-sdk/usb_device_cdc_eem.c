...
USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) static usb_device_cdc_eem_struct_t
    g_cdcEemHandle[USB_DEVICE_CONFIG_CDC_EEM];

static usb_status_t USB_DeviceCdcEemAllocateHandle(usb_device_cdc_eem_struct_t **handle) {
    uint32_t count;
...
static usb_status_t USB_DeviceCdcEemFreeHandle(usb_device_cdc_eem_struct_t *handle) {
    handle->handle = NULL;
...
static usb_status_t USB_DeviceCdcEemEndpointsDeinit(usb_device_cdc_eem_struct_t *cdcEemHandle) {
    ...
    for (count = 0; count < cdcEemHandle->dataInterfaceHandle->endpointList.count; ++count) {
        error = USB_DeviceDeinitEndpoint(
            cdcEemHandle->handle, cdcEemHandle->dataInterfaceHandle->endpointList.endpoint[count].endpointAddress);
    }
...
static usb_status_t USB_DeviceCdcEemBulkIn(
    ...
    if (cdcEemHandle->configStruct && cdcEemHandle->configStruct->classCallback) {
        status = cdcEemHandle->configStruct->classCallback((class_handle_t)cdcEemHandle, kUSB_DeviceEemEventSendResponse, message);
    }
...
static usb_status_t USB_DeviceCdcEemBulkOut(
    ...
    if (cdcEemHandle->configStruct && cdcEemHandle->configStruct->classCallback) {
        status = cdcEemHandle->configStruct->classCallback((class_handle_t)cdcEemHandle, kUSB_DeviceEemEventRecvResponse, message);
    }

    return status;
}

static usb_status_t USB_DeviceCdcEemEndpointsInit(usb_device_cdc_eem_struct_t *cdcEemHandle) {
    ...
    if ((cdcEemHandle->configuration == 0) || (cdcEemHandle->configuration > cdcEemHandle->configStruct->classInfomation->configurations)) {
        return error;
    }

    interfaceList = &cdcEemHandle->configStruct->classInfomation->interfaceList[cdcEemHandle->configuration - 1];
    for (count = 0; count < interfaceList->count; ++count) {
        if (interfaceList->interfaces[count].classCode == USB_DEVICE_CONFIG_CDC_COMM_CLASS_CODE &&
            interfaceList->interfaces[count].subclassCode == USB_DEVICE_CONFIG_CDC_EEM_SUBCLASS_CODE) {
            for (index = 0; index < interfaceList->interfaces[count].count; index++) {
                if (interfaceList->interfaces[count].interface[index].alternateSetting == cdcEemHandle->alternate) {
                    interface = &interfaceList->interfaces[count].interface[index];
                    break;
                }
            }
            cdcEemHandle->interfaceNumber = interfaceList->interfaces[count].interfaceNumber;
            break;
        }
    }
    if (interface == NULL) {
    ...
    for (count = 0; count < interface->endpointList.count; ++count) {
        ...
        epInitStruct.zlt = 0;
        epInitStruct.transferType = interface->endpointList.endpoint[count].interval;
        epInitStruct.endpointAddress = interface->endpointList.endpoint[count].endpointAddress;
        epInitStruct.maxPacketSize = interface->endpointList.endpoint[count].maxPacketSize;
        epInitStruct.transferType = interface->endpointList.endpoint[count].transferType;

        if ((((epInitStruct.endpointAddress & USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) >> USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT) == USB_IN) &&
            (epInitStruct.transferType == USB_ENDPOINT_BULK)) {
            // TODO(atv): ACM fills in their pipe struct here
            cdcEemHandle->bulkIn.isBusy = 0;
            epCallback.callbackFn = USB_DeviceCdcEemBulkIn;
        } else if ((((epInitStruct.endpointAddress & USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) >> USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT) == USB_OUT) &&
            (epInitStruct.transferType == USB_ENDPOINT_BULK)) {
            // TODO(atv): ACM fills in their pipe struct here
            cdcEemHandle->bulkOut.isBusy = 0;
        ...
        error = USB_DeviceInitEndpoint(cdcEemHandle->handle, &epInitStruct, &epCallback);
    }

    return error;
}

usb_status_t USB_DeviceCdcEemInit(uint8_t controllerId, usb_device_class_config_struct_t *config, class_handle_t *handle) {
    usb_device_cdc_eem_struct_t *cdcEemHandle;
    usb_status_t error;
...
usb_status_t USB_DeviceCdcEemEvent(void *handle, uint32_t event, void *param) {
    ...
    switch (eventCode) {
        ...
        case kUSB_DeviceClassEventSetInterface:
            ...
            if (cdcEemHandle->interfaceNumber != ((uint8_t)(interfaceAlternate >> 8))) {
                break;
            }
...
usb_status_t USB_DeviceCdcEemSend(class_handle_t handle, uint8_t ep, uint8_t *buffer, uint32_t length) {
    usb_device_cdc_eem_struct_t *cdcEemHandle;
    usb_status_t status = kStatus_USB_Error;
...
usb_status_t USB_DeviceCdcEemRecv(class_handle_t handle, uint8_t ep, uint8_t *buffer, uint32_t length) {
    usb_device_cdc_eem_struct_t *cdcEemHandle;
    usb_status_t status;
