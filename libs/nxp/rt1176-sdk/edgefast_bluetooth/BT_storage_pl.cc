...
API_RESULT storage_open_pl(UCHAR type, UCHAR mode) {
  if (NULL != fp[type]) {
    valiant::filesystem::Close(fp[type]);
    fp[type] = NULL;
  }
  bool open;
  if (mode == STORAGE_OPEN_MODE_WRITE) {
    open = valiant::filesystem::Open(&lfs_file[type], (char *)fn[type], true);
  } else {
    open = valiant::filesystem::Open(&lfs_file[type], (char *)fn[type]);
  }
  if (!open) {
    return API_FAILURE;
  }
...
INT16 storage_write_pl(UCHAR type, void *buffer, UINT16 size) {
  ...
  if (NULL != fp[type]) {
    nbytes = (INT16)valiant::filesystem::Write(fp[type], buffer, size);
  }
  return nbytes;
}
...
INT16 storage_read_pl(UCHAR type, void *buffer, UINT16 size) {
  INT16 nbytes = 0;
  if (NULL != fp[type]) {
    nbytes = (INT16)valiant::filesystem::Read(fp[type], buffer, size);
  }
  return nbytes;
}
...
INT16 storage_write_signature_pl(UCHAR type) {
  INT16 nbytes = 0;
  if (NULL != fp[type]) {
    nbytes = (INT16)valiant::filesystem::Write(fp[type], ssign[type],
                                               STORAGE_SKEY_SIZE);
  }
  return nbytes;
}
...
INT16 storage_read_signature_pl(UCHAR type) {
  ...
  if (NULL != fp[type]) {
    nbytes = valiant::filesystem::Read(fp[type], sign, STORAGE_SKEY_SIZE);
