...
int settings_load_one(const char *name, void *value, size_t val_len) {
  lfs_file_t readFile;
  bool success = valiant::filesystem::Open(&readFile, name);
  if (!success) {
    return -1;
  }

  int len = valiant::filesystem::Size(&readFile);
  if (value != NULL) {
    len = valiant::filesystem::Read(&readFile, value, val_len);
  }
  valiant::filesystem::Close(&readFile);
  return len;
}
...
int settings_load_subtree_direct(const char *subtree,
    ...
    bool success = valiant::filesystem::Stat(subtree, &info);
    if (!success) {
        return -1;
    }
    if (info.type == LFS_TYPE_DIR)
    {
        success = valiant::filesystem::DirOpen(&dir, subtree);
        if (!success) {
            return -1;
        }

        /* iterate until end of directory */
        while ((err = valiant::filesystem::DirRead(&dir, &info)) != 0)
        {
        ...
        valiant::filesystem::DirClose(&dir);
    }
    else
    {
...
static int settings_load_subtree_scan(struct settings_handler_static *set,
  ...
  memset((void *)&dir, 0, sizeof(dir));
  ret = valiant::filesystem::Stat(key, &info);
  if (!ret) {
    return -EIO;
  }
  if (info.type == LFS_TYPE_DIR) {
    ...
    ret = valiant::filesystem::DirOpen(&dir, key);
    if (!ret) {
      return -EIO;
    }
    while ((err = valiant::filesystem::DirRead(&dir, &info)) != 0) {
    ...
    ret = valiant::filesystem::DirClose(&dir);
    if (!ret) {
      return -EIO;
    }
  } else {
...
static int settings_check_create_path(const char *name) {
  ...
  while (index > 0) {
    ...
    dname[index] = 0;
    bool success = valiant::filesystem::MakeDirs(dname);
    if (!success) {
      return -1;
    }
    index = settings_has_spacer(name, SETTINGS_NAME_SEPARATOR, times++);
...
int settings_save_one(const char *name, const void *value, size_t val_len) {
  ...
  bool success = valiant::filesystem::Open(&writeFile, name, true);
  if (!success) {
    return -1;
  }

  int len = valiant::filesystem::Write(&writeFile, value, val_len);
  success = valiant::filesystem::Close(&writeFile);
  if (!success) {
    return -1;
  }
...
int settings_delete(const char *name) {
  if (name) {
      if (valiant::filesystem::Remove(name)) {
          return 0;
      }
  }
  return -1;
